...
export function Header({ done, searchQuery, onSearchChange }: Props) {
  ...
  async function handleGlobalTest(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    const doneRules: { rule: import('../../data/grammar').Rule; level: import('../../data/grammar').Level; category: import('../../data/grammar').Category }[] = [];
    DATA.forEach(lvl => {
    ...
    <header>
      <div className="header-top">
        <div className="title-block">
          <h1>English <span>Grammar</span> Tree</h1>
        ...
        <div className="overall-progress">
          ...
          <div className="overall-label">из <span>{total}</span> правил</div>
      ...
      <div className="header-container">
        <div className="stats-row">
          <div className="stat-chip">Прогресс: <strong>{pct}%</strong></div>
          <div className="stat-chip">Осталось: <strong>{total - checked}</strong> правил</div>
          {completedLevels.length > 0 && (
            <div className="stat-chip">✓ Завершены: <strong>{completedLevels.map(l => l.id).join(', ')}</strong></div>
          )}
        </div>
