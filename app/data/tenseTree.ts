...
export const TENSE_TREE: Record<string, TenseNode> = {
  ...
  present_now: { q: 'Это постоянное состояние или процесс прямо сейчас?', choices: [
    { label: 'Постоянное (знаю, люблю, хочу...)', icon: '💭', result: 'present_simple' },
    { label: 'Процесс сейчас / временно', icon: '⚡', result: 'present_continuous' },
  ]},
  present_perfect_branch: { q: 'Важна продолжительность или сам факт?', choices: [
    { label: 'Сам факт / результат / опыт', icon: '✅', result: 'present_perfect' },
    { label: 'Продолжительность ("как долго")', icon: '⏳', result: 'present_perfect_continuous' },
  ]},
  past: { q: 'Что ты хочешь выразить о прошлом?', choices: [
  ...
  past_perfect_branch: { q: 'Нужно подчеркнуть продолжительность?', choices: [
  ...
  future: { q: 'Как ты выражаешь будущее?', choices: [
